...
<div id="dvmaincontainer">
 ...
 <div class="rightX">
		...
		<h4>You can also have are lists to see the structure compared below.</h4>
		</div>
		<div class="rContent">
			<div class="productX">
	...
	<p> 
	    <h3> You can see the simple contain here and choose your things~ 	</h3>			
...
<div class="productX">		
    ...
    dao.queryAllPro(proList); 
    for (int i = 0;i <proList.size();i++){%>
    <form action="./showDetail" method="post">
   
      picture:     <img src="<%=proList.get(i).getAddress() %>"height="194"><br/>
    <%=proList.get(i).getAddress() %>
    color:<%=proList.get(i).getColor() %><br/>
    height:<%= proList.get(i).getHeight() %>
    price: <%=proList.get(i).getPrice() %>
    <input type="HIDDEN" name="id" value="<%=proList.get(i).getId()%>"><br/>
    <input type="SUBMIT" name="submit" value="Details"><br/>
    </form>
    <%} %>
</div>
		
	<a href="#">  <img src="images/sp01.jpg" width="66" height="64"  alt=""/>CLICK HERE ! Be sure to add into the v-mall! </a>
